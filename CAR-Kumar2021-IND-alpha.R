...
distanceModelCode <- nimbleCode({
  ...
  for (i in 1:I) { # Loop through each transect
    ...
    lam[i] <- nrep * z_site[i] * exp(alpha[i]) # multiply abundance (z) of each grid with proportion that each grid contribute to the transect and site-specific alpha
  }
...
tracked_var <- c("sigma", "p", "muc", "gs_k", "sigma0", "pi", "fix_z", "z", "spatial_z", "lam",
