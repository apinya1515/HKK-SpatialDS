...
distanceModelCode <- nimbleCode({
  ...
  spatial_z[1:L] ~ dcar_normal(
    adj = adj[1:njoin], weights = weights[1:njoin],
    num = num[1:L], tau = tau, zero_mean = 1
  ) # random spatial effect (intercept) constrained to sum-to-zero
  ...
  for (i in 1:I) { # Loop through each transect
    # z_site[i] is proportionated density along transect (groups / sq km)
    z_site[i] <- inprod(propM[i, 1:L], z[1:L])
    ...
    lam[i] <- nrep * z_site[i] * area_tr[i] * exp(alpha[i])
  }
...
tracked_var <- c(
  "sigma", "p", "muc", "gs_k", "sigma0", "pi", "fix_z", "z", "spatial_z", "lam",
