...
data_tr <- read.table('D:\\2024SpatialDistance\\analysis\\line_data.txt', 
                       sep='\t', header=T)
#

# landscape data
#data_land <- read.csv('D:\\UserData\\KUDrive\\@Projects\\Kluay_Line_Transect\\HKK_Cov1sqkm_.csv')
data_land <- read.csv('D:\\2024SpatialDistance\\analysis\\HKK_Cov1sqkm_.csv')
...
data_prop <- read.csv('D:\\2024SpatialDistance\\analysis\\TRidentity.csv')
##############################################################################
### Input parameters
species <- 'SBR'
...
distanceModelCode <- nimbleCode({
  ...
  beta0 ~ dnorm(0, 0.001) # nonspatial intercept
  ...
  for(l in 1:L) {
    ...
    log_lam[l] <- inprod(covar[l, 1:n_covar], beta[1:n_covar]) + beta0 # regression for log abundance (without intercept)
...
inits <- list(muc = runif(1, 1, 3), sigma0 = runif(1, 1, 2), p = runif(1, 0.5, 1),
              beta = rnorm(ncol(covar), 0,1), beta0 = runif(1, 0, 0.1))
...
mcmcConf <- configureMCMC(distanceModel, monitors = c("sigma", "p", "gs", "sigma0", "pi", 'lam', 'beta'))
...
###summary(samples)
#adjust for test MCMC =600 , 500
samples <- runMCMC(Cmcmc, niter = 600, nburnin = 500, thin = 5, nchains = 1)
###summary(samples)
...
library(terra)
library(spdep)
poly <- st_read('D:\\2024SpatialDistance\\analysis\\HKK1sqkmGrid.shp')
