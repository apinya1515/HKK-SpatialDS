...
dist_class_n <- 4 # number of dist classes
gsBreaks <- c(1, 2, 7 ,30) # upper breaks group size of each gs classes - c(1, 3) = 2 classes 1st = 1, 2nd >= 2
##############################################################################
...
sum(pi)
# เติมค่าให้ครบ 6 ค่า
#pi_val <- c(pi_val, rep(mean(pi_val), 2))  # เติมด้วยค่าเฉลี่ยของ pi_val
# ตรวจสอบขนาด
#length(pi_val)  # ตอนนี้จะมี 6 ค่าแล้ว
# สร้าง matrix
#pi <- matrix(pi_val, nrow = 6, byrow = F)
...
#Visualize pi#Apinya
matplot(t(pi), type = 'l', lty = 1:gs_class_n, col = 1:gs_class_n,
        xlab = 'Distance Class', ylab = 'Probability', main = 'Multinomial Prob')
legend('topright', legend = as.character(1:gs_class_n), title = 'Group size', lty = 1:gs_class_n, col = 1:gs_class_n)
...
#Apinya
sigma_names <- grep("^sigma\\[[0-9]+\\]$", colnames(samples), value = TRUE)  # หาชื่อ sigma[1], sigma[2], sigma[3], sigma[4]
gs_class_n <- length(sigma_names)  # ตั้งตามที่มีจริง
gx_list <- list()

for (i in 1:gs_class_n) {
  sigma_i <- median(samples[, sigma_names[i]])
  gx_list[[i]] <- exp((-1 * x_seq^2) / (2 * sigma_i^2))
}


# เตรียมตัวแปร
x_seq <- seq(0, dist_limit, length.out = 500)  # ใช้เส้นเรียบขึ้น
colors <- c('red', 'green', 'blue', 'purple','black','yellow')  # สีแยกแต่ละ group size class
ltys <- 1:4
gx_list <- list()# คำนวณ gx สำหรับทุก group size class
for (i in 1:gs_class_n) {
  sigma_i <- median(samples[, paste0("sigma[", i, "]")])
  gx_list[[i]] <- exp((-1 * x_seq^2) / (2 * sigma_i^2))
}
# สร้าง histogram
hp <- hist(data_sub$P.dist,
           breaks = dist_class_n,
           main = 'Histogram & Detection function',
           xlab = 'Distance')

# วาด detection function ทับ histogram
for (i in 1:gs_class_n) {
  lines(x_seq, gx_list[[i]] * max(hp$counts),
        col = colors[i], lwd = 1, lty = ltys[i])
}

# เพิ่ม legend
legend('topright',
       legend = paste('GS class', 1:gs_class_n),
       title = 'Group Size Class',
       col = colors,
       lty = ltys)
