...
#data_tr <- read.table('D:\\UserData\\Dropbox\\@KU_pop_course\\day1_exercise\\line_data.txt',                 sep='\t', header=T)
data_tr <- read.csv('D:\\HKK-SpatialDS\\analysis\\hkkdata.csv')#Apinya
# landscape data
#data_land <- read.csv('D:\\UserData\\KUDrive\\@Projects\\HKK_Line_Transect\\HKK_Cov1sqkm_.csv')
data_land <- read.csv('D:\\HKK-SpatialDS\\analysis\\HKK_Cov1sqkm_15042025.csv')#Apinya
# transect X landscape data
#data_prop <- read.csv('D:\\UserData\\KUDrive\\@Projects\\HKK_Line_Transect\\TRidentity.csv')
data_prop <- read.csv('D:\\HKK-SpatialDS\\analysis\\TRidentity.csv')#Apinya
...
poly <- st_read('D:\\HKK-SpatialDS\\analysis\\HKK1sqkmGrid.shp')#Apinya
...
path <- 'D:/HKK-SpatialDS/analysis/Archive R code/'
source(paste0(path, 'model-detection-function-grsize-landscape-CAR-Kumar2021-03082025.R'))
...
samples_chains <- runMCMC(Cmcmc, niter = 5000, nburnin = 500, thin = 2, nchains = 2)

(t_elapse <- Sys.time() - t_start) # time elapse                        ###summary(samples_chains)
