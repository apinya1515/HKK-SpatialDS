...
samples <- runMCMC(Cmcmc, niter = 60, nburnin = 50, thin = 5, nchains = 1)
summary(samples)
################################ Post training ############################
...
sigma_7 <- median(samples[,'sigma[7]'])
sigma_8 <- median(samples[,'sigma[8]'])
...
legend('topright', legend=c('1','2','3','4','5','6','7','8'), title = 'Group size', lty=c(1,2,3,4,5,6,7,8))
...
gx_5 <- exp((-1*x_seq^2)/(2*sigma_5^2))
gx_6 <- exp((-1*x_seq^2)/(2*sigma_6^2))
gx_7 <- exp((-1*x_seq^2)/(2*sigma_7^2))
gx_8 <- exp((-1*x_seq^2)/(2*sigma_8^2))
...
lines(x_seq, gx_4*max(hp$counts), col='pink', lwd=2)
lines(x_seq, gx_5*max(hp$counts), col='purple', lwd=2)
lines(x_seq, gx_6*max(hp$counts), col='yellow', lwd=2)
lines(x_seq, gx_7*max(hp$counts), col='brown', lwd=2)
lines(x_seq, gx_8*max(hp$counts), col='orange', lwd=2)


legend('topright', legend=c('1','2','3','4','5','6','7','8'), 
title = 'Group size', col=c('red', 'green', 'blue','pink','purple','yellow','brown','orange')
, lty=1)
