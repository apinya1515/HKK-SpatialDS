# run_pig_selection.R
# Backward stepwise variable selection using wAIC in NIMBLE for PIG (Wild boar)
# Run parallelized evaluations across 4 cores to speed up selection.
...
cat("Initializing cluster of 4 workers for parallel model runs...\n")
cl <- makeCluster(4)

# Export variables to workers
clusterExport(cl, c("sp", "nrep", "dist_limit", "gsBreaks", 
...
# Process and sort all evaluated models for PIG
...
df_models <- df_models %>% filter(is.finite(wAIC)) %>% arrange(wAIC)
...
cat("\nTop models for Wild boar:\n")
print(top_5_df)

# Merge and append to Model_Selection_Summary.csv
summary_file <- "Results/Model_Selection_Summary.csv"
if (file.exists(summary_file)) {
  existing_summary <- read.csv(summary_file, stringsAsFactors = FALSE)
  # Remove PIG if somehow present
  existing_summary <- existing_summary %>% filter(Species != sp_name)
  final_summary <- rbind(existing_summary, top_5_df)
} else {
  final_summary <- top_5_df
}

write.csv(final_summary, summary_file, row.names = FALSE)
cat(sprintf("\nResults successfully merged and saved to '%s'\n", summary_file))
