...
# Cap physically impossible NDVI CV values at 0.5 before scaling to prevent outlier distortion
data_land$ndvi_cv[data_land$ndvi_cv > 0.5] <- 0.5
...
y_matrix <- array(0,
  dim = c(gs_class_n, dist_class_n, tran_n),
  dimnames = list(1:gs_class_n, distBreaks, tran_id)
) # 3 dimensions = group size, distance, transect
